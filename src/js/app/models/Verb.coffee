define [
  ...
  Verb = can.Model.extend(
    validTenseSubjectsMap: (->
      ...
      res = {}
      subjectsCopy = undefined
      i = 0
      len = tenses.length

      while i < len
        subjectsCopy = subjects.slice(0)
        if tenses[i] is "imperativo"
          subjectsCopy = [
            "tu"
            "lui"
            "noi"
    ...
    conjugators:
      presente: (subject, infinitive, type, root, isReflexive) ->
        ...
        isGareOrCareVerb = (infinitive.slice(-4) is "care" or infinitive.slice(-4) is "gare")
        ...
        return irregulars[infinitive][subject]  if irregulars[infinitive] isnt `undefined` and irregulars[infinitive][subject] isnt `undefined`
        if subject is "tu" or subject is "noi"
          if hasIBeforeEnding
            root = root.slice(0, -1)
          else root = root + "h"  if isGareOrCareVerb
      ...
      "passato prossimo": (subject, infinitive, type, root, isReflexive) ->
        ...
        useEssere = isReflexive or Verb.conjugatorData.essereVerbsPassatoProssimo.indexOf(infinitive) isnt -1
        usePlural = useEssere and [
          "noi"
          "voi"
          "loro"
        ].indexOf(subject) isnt -1
        ...
        auxilary = new Verb((if useEssere then infinitive: "essere" else infinitive: "avere")).conjugate(subject, "presente")
    ...
    findAll: (params) ->
      ...
      $.getJSON "https://api.quizlet.com/2.0/sets?set_ids=" + setList + "&client_id=AEgzk2BXWu&callback=?", (data) ->
        ...
        $.each data[0].terms, (index, term) ->
          ...
          @infinitive = @infinitive.replace(/(.*)(are|ere|ire|si)\b(.*)$/, "$1$2")  if [
            "are"
            "ere"
            "ire"
            "rsi"
          ].indexOf(term.infinitive.slice(-3)) is -1
          if Verb.conjugatorData.irregularPastParticiples[term.infinitive] isnt `undefined` or Verb.conjugatorData.irregularPresentVerbs[term.infinitive] isnt `undefined` or Verb.conjugatorData.irregularImperfectVerbs[term.infinitive] isnt `undefined` or Verb.conjugatorData.irregularImperativeVerbs[term.infinitive] isnt `undefined`
            term.irregular = true
            irregulars.push term
          else
    ...
    findBy: (property, matches, callback) ->
      ...
      @findAll({}).then((verbs) ->
        verbs.each (verb) ->
          res.push verb  if matches.indexOf(verb[property]) isnt -1
  ...
  ,
    ...
    conjugate: (subject, tense) ->
      ...
      throw new Error("Invalid tense. Tense must be one of: " + Object.keys(tenseMap).join(", ") + ".")  if tenseMap[tense] is `undefined`
      throw new Error("Invalid subject for the tense: " + tense + ". Subject must be one of: " + tenseMap[tense].join(", ") + ".")  if tenseMap[tense].indexOf(subject) is -1
